 ...
 *         confirmation form is generated by confirm_form().
 *    'candidates' => A function that will return an array of replacement candidates
 *         for when the user has permissions to change the referenced entity in all
 *         referring entities.  The returned array should be key => value pairs, where
 *         the key is the possible replacement entity id, and the value is a human-readable
 *         title of the option.  This function will be passed the arguments:
 *            - string $entity_type The entity type (eg., 'node', 'user', 'taxonomy_term').
 *            - string $bundle The bundle for the entity type.
 *            - array $exclude_ids An array of ids to exclude from the returned options array.
 *
 *   @see erri.inc
 */
function hook_erri_info() {
  return array(
    'my_delete_form_using_confirm_form' => array(
      ...
      'candidates' => 'my_function_to_get_replacement_options',
    ),
